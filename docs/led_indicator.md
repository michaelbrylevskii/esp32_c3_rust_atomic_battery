...
То есть уже сейчас можно описывать переходы и "пульсации" на уровне API, даже если физически GPIO-led покажет только дискретное включение и выключение.
...
Это включит только красный LED.
...
Это полезно, когда нужны не просто blink/factory-хелперы, а более точная анимация.
...
С цифровым backend'ом это будет выглядеть как ступенчатое переключение, но с PWM backend'ом тот же паттерн сможет стать реальной плавной пульсацией.
...
- later можно захотеть заменить простой GPIO-led на PWM backend без переделки прикладной логики
