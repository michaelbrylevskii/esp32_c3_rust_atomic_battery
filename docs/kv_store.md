...
Модуль [`kv_store`](../src/common/utils/kv_store.rs) — утилитарный слой для хранения типизированных key-value данных.

Он используется как текстовый прикладной формат `KV1`.

Роли слоёв:

- NFC-обёртка отвечает за чтение и запись payload
...
- парсить `KV1` обратно
- поддерживать строки, целые числа, числа с плавающей точкой и boolean
...
- первая строка `KV1` — сигнатура формата
- дальше каждая строка имеет вид `key=TYPE:value`
