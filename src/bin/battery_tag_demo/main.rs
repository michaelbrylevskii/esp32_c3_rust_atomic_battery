...
use common::drivers::nfc_tag::sync_nfc::{NfcError, TagInfo};
...
const DEMO_CAPACITY: u64 = 600_000;
const DEMO_CHARGE: u64 = 600_000;
...
fn main() {
    ...
    info!("Starting battery_tag_demo binary");
...
fn run() -> Result<(), DemoError> {
    ...
    let expected = BatteryTag::new(DEMO_CAPACITY, DEMO_CHARGE)?;
        ...
        match nfc.poll_tag(POLL_TIMEOUT) {
            Ok(Some(tag_info)) => {
                ...
                if last_seen_uid_hex.as_deref() != Some(uid_hex.as_str()) {
                    ...
                    write_and_validate_battery_tag(&mut nfc, &tag_info, &expected)?;
                }
            ...
            Err(err) => {
                warn!("PN532 error while polling battery tag: {err}");
...
fn write_and_validate_battery_tag(
    ...
    match parsed {
        AtomicTag::Battery(actual) if actual == *expected => {
            info!(
                "Battery tag {:02X?} validated: capacity={}, charge={}, healthy={}, dirty={}, session_id={}",
                tag_info.uid,
                actual.capacity,
                actual.charge,
                actual.healthy,
        ...
        AtomicTag::Battery(actual) => {
            warn!("Read-back differs from expected battery tag: {:?}", actual);
            Err(DemoError::Validation(
                "battery tag read-back differs from expected",
            ))
        }
        AtomicTag::Service(_) => Err(DemoError::Validation(
            "expected battery tag, got service tag after read-back",
        )),
    }
}
